...
Write-Host "  SUCCESS  Your Marketplace App (RabbitMQ Server (Community) + Management plugin) has been deployed successfully!" -ForegroundColor Green
Write-Host "================================================================================" -ForegroundColor Green
Write-Host ""
...
Unregister-ScheduledTask -TaskName 'markgen-windows-2025-cleanup' -Confirm:$false -ErrorAction SilentlyContinue
# Also clear any legacy RunOnce entry from older installs (harmless if absent).
Remove-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce' -Name 'windows-2025_cleanup' -ErrorAction SilentlyContinue
