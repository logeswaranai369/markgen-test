...
Write-Host "#            Your Marketplace App (WordPress on IIS (Windows)) has been deployed successfully!            #" -ForegroundColor Red
...
Write-Host "Credential: mariadb_root_password (stored at C:\credentials.txt)" -ForegroundColor Red
if (Test-Path 'C:\credentials.txt') { Get-Content 'C:\credentials.txt' }
Write-Host ""
Write-Host "Credential: wp_db_password (stored at C:\credentials.txt)" -ForegroundColor Red
if (Test-Path 'C:\credentials.txt') { Get-Content 'C:\credentials.txt' }
Write-Host ""
Write-Host "Credential: wp_admin_password (stored at C:\credentials.txt)" -ForegroundColor Red
if (Test-Path 'C:\credentials.txt') { Get-Content 'C:\credentials.txt' }
Write-Host ""
